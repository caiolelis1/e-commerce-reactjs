...
const Checkout = ({ cart, order, onCaptureCheckout, error }) => {
    ...
    let Confirmation = () => order.customer ? (
        <>
            <div>
                <Typography variant="h5" >Obrigado pela sua compra, {order.customer.firstname} {order.customer.lastmame}!</Typography>
                <Divider className={classes.divider} />
                <Typography variant="subtitle2">Número da compra: {order.customer.reference}</Typography>
            </div>
            <br />
            <Button component={Link} to="/" variant="outlined" type="button" >Voltar para home</Button>
        </>
    ) : isFinished ? (
        <>
            <div>
